...
function getWeekNumberByDate(date) {
  const nDate = date;
  let bonus = 0;
  const startDate = new Date(Date.UTC(nDate.getFullYear(), 0, 1));
  ...
  const week =
    Math.ceil((nDate - startDate + 3600000 * 24) / (1000 * 60 * 60 * 24 * 7)) +
    bonus;
  return week;
}
